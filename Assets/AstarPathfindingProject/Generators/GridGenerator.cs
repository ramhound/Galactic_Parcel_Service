	...
	,IRaycastableGraph
	{
		...
		protected bool ClipLineSegmentToBounds ( Vector3 a, Vector3 b, out Vector3 outA, out Vector3 outB ) {
			...
			if (a.x < 0 || a.z < 0 || a.x > width || a.z > depth ||
				...
				intersection = Polygon.SegmentIntersectionPoint (a,b,p1,p2, out intersect);

				if (intersect) {
					intersectCount++;
					if (!Polygon.Left (p1,p2,a)) {
						a = intersection;
					} else {
						b = intersection;
					}
				}
				intersection = Polygon.SegmentIntersectionPoint (a,b,p2,p3, out intersect);

				if (intersect) {
					intersectCount++;
					if (!Polygon.Left (p2,p3,a)) {
						a = intersection;
					} else {
						b = intersection;
					}
				}
				intersection = Polygon.SegmentIntersectionPoint (a,b,p3,p4, out intersect);

				if (intersect) {
					intersectCount++;
					if (!Polygon.Left (p3,p4,a)) {
						a = intersection;
					} else {
						b = intersection;
					}
				}
				intersection = Polygon.SegmentIntersectionPoint (a,b,p4,p1, out intersect);

				if (intersect) {
					intersectCount++;
					if (!Polygon.Left (p4,p1,a)) {
						a = intersection;
					} else {
						b = intersection;
					}
