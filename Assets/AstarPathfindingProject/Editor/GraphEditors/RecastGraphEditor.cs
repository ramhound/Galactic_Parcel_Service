...
namespace Pathfinding {
	...
	public class RecastGraphEditor : GraphEditor {
		...
		void UpdateDebugMeshes () {
			...
			if (tiles != null) {
				...
				for (int i = 0; i < tiles.Length; i++) {
					if (i < gizmoMeshes.Count && gizmoMeshes[i].tile == tiles[i]) {
						// Tile is still valid, don't update it
					} else {
		...
		static Mesh CreateNavmeshSurfaceVisualization (RecastGraph.NavmeshTile tile) {
			var mesh = new Mesh();
			...
			for (int j = 0; j < tile.nodes.Length; j++) {
				...
				for (int v = 0; v < 3; v++) {
					colors[node.GetVertexArrayIndex(v)] = (Color32)AstarColor.GetAreaColor(node.Area);
				}
			}
